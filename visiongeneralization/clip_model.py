...
class CLIPCoco(torch.nn.Module):
    def __init__(self, embed_dim=512, transformer_embed_dim=768,
                 text_model="distilbert-base-multilingual-cased", max_length=128) -> None:
        super().__init__()
        self.vision_encoder = VisionEncoder(embed_dim)
        self.caption_encoder = TextEncoder(embed_dim, transformer_embed_dim, text_model)
        self.tokenizer = Tokenizer(AutoTokenizer.from_pretrained(text_model), max_length)
